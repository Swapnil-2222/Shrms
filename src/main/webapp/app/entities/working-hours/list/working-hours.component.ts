...
import { EntityArrayResponseType, WorkingHoursService } from '../service/working-hours.service';
...
export class WorkingHoursComponent implements OnInit {
  ...
  trackId = (_index: number, item: IWorkingHours): number => this.workingHoursService.getWorkingHoursIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.workingHours = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IWorkingHours[] | null): IWorkingHours[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.workingHoursService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
