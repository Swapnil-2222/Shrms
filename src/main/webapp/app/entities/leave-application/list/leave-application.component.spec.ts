...
describe('LeaveApplication Management Component', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [
        RouterTestingModule.withRoutes([{ path: 'leave-application', component: LeaveApplicationComponent }]),
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.leaveApplications?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to leaveApplicationService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getLeaveApplicationIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getLeaveApplicationIdentifier).toHaveBeenCalledWith(entity);
