...
type RestOf<T extends ICompany | NewCompany> = Omit<T, 'lastModified'> & {
...
export class CompanyService {
  ...
  protected convertDateFromClient<T extends ICompany | NewCompany | PartialUpdateCompany>(company: T): RestOf<T> {
    return {
      ...company,
      lastModified: company.lastModified?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCompany: RestCompany): ICompany {
    return {
      ...restCompany,
      lastModified: restCompany.lastModified ? dayjs(restCompany.lastModified) : undefined,
    };
  }
