...
import com.techvg.shrms.repository.StateRepository;
import com.techvg.shrms.service.criteria.StateCriteria;
import com.techvg.shrms.service.dto.StateDTO;
import com.techvg.shrms.service.mapper.StateMapper;
...
class StateResourceIT {
    ...
    void createState() throws Exception {
        ...
        assertThat(testState.getStatus()).isEqualTo(DEFAULT_STATUS);
    ...
    @Test
    @Transactional
    void getAllStatesByStatusIsEqualToSomething() throws Exception {
        // Initialize the database
        stateRepository.saveAndFlush(state);

        // Get all the stateList where status equals to DEFAULT_STATUS
        defaultStateShouldBeFound("status.equals=" + DEFAULT_STATUS);

        // Get all the stateList where status equals to UPDATED_STATUS
        defaultStateShouldNotBeFound("status.equals=" + UPDATED_STATUS);
    }

    @Test
    @Transactional
    void getAllStatesByStatusIsInShouldWork() throws Exception {
        // Initialize the database
        stateRepository.saveAndFlush(state);

        // Get all the stateList where status in DEFAULT_STATUS or UPDATED_STATUS
        defaultStateShouldBeFound("status.in=" + DEFAULT_STATUS + "," + UPDATED_STATUS);

        // Get all the stateList where status equals to UPDATED_STATUS
        defaultStateShouldNotBeFound("status.in=" + UPDATED_STATUS);
    }

    @Test
    @Transactional
    void getAllStatesByStatusIsNullOrNotNull() throws Exception {
        // Initialize the database
        stateRepository.saveAndFlush(state);

        // Get all the stateList where status is not null
        defaultStateShouldBeFound("status.specified=true");

        // Get all the stateList where status is null
        defaultStateShouldNotBeFound("status.specified=false");
    }

    @Test
    @Transactional
    void getAllStatesByStatusContainsSomething() throws Exception {
        // Initialize the database
        stateRepository.saveAndFlush(state);

        // Get all the stateList where status contains DEFAULT_STATUS
        defaultStateShouldBeFound("status.contains=" + DEFAULT_STATUS);

        // Get all the stateList where status contains UPDATED_STATUS
        defaultStateShouldNotBeFound("status.contains=" + UPDATED_STATUS);
    }

    @Test
    @Transactional
    void getAllStatesByStatusNotContainsSomething() throws Exception {
        // Initialize the database
        stateRepository.saveAndFlush(state);

        // Get all the stateList where status does not contain DEFAULT_STATUS
        defaultStateShouldNotBeFound("status.doesNotContain=" + DEFAULT_STATUS);

        // Get all the stateList where status does not contain UPDATED_STATUS
        defaultStateShouldBeFound("status.doesNotContain=" + UPDATED_STATUS);
    }
    ...
    void putExistingState() throws Exception {
        ...
        assertThat(testState.getStatus()).isEqualTo(UPDATED_STATUS);
    ...
    void partialUpdateStateWithPatch() throws Exception {
        ...
        partialUpdatedState.stateName(UPDATED_STATE_NAME).lgdCode(UPDATED_LGD_CODE);
