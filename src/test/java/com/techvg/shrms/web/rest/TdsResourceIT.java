...
import com.techvg.shrms.repository.TdsRepository;
import com.techvg.shrms.service.criteria.TdsCriteria;
import com.techvg.shrms.service.dto.TdsDTO;
import com.techvg.shrms.service.mapper.TdsMapper;
...
class TdsResourceIT {
    ...
    void createTds() throws Exception {
        ...
        restTdsMockMvc
            .perform(
                post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(tdsDTO))
            )
    ...
    void putWithMissingIdPathParamTds() throws Exception {
        ...
        restTdsMockMvc
            .perform(
                put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(tdsDTO))
            )
    ...
    void deleteTds() throws Exception {
        ...
        // Delete the tds
        restTdsMockMvc
            .perform(delete(ENTITY_API_URL_ID, tds.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
